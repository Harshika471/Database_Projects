...
def most_popular_cuisine(my_collection):
    ...
    count_rows = my_collection.count()
    #define an empty list
    popular_cuisine=[]
    for value in my_collection.aggregate([
        {"$group": {"_id": "$cuisine","count": { "$sum": 1 }}},{"$sort":{"count":-1}},
        {"$project": {"percentage": {"$multiply": [{"$divide": [100,count_rows ]}, "$count"]}}}
    ]):
...
def ratio_per_borough_and_cuisine(my_collection, cuisine):
    ...
    key_value_store = [[key, val] for key, val in empty_dict.items()]
...
def best_restaurants(my_collection, cuisine, borough, zipcode):
    ...
    for j in range(len(my_list)):
        ...
        customer_reviews.append(my_list[j]['Average'])
    return(best_restro,customer_reviews)
